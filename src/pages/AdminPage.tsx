...
import { 
  ...
  Database, Laptop} from "lucide-react";
...
const AdminPage = () => {
        ...
        return (
          <>
            <div className="grid grid-cols-4 gap-4 mb-6">
              <StatCard label="Total Revenue" value="$58,947" percentage={75} color="from-indigo-600 to-blue-500" />
              <StatCard label="Today's Sales" value="127" percentage={55} color="from-teal-500 to-emerald-400" />
              <StatCard label="Conversion" value="0.58%" percentage={45} color="from-fuchsia-600 to-pink-500" />
              <StatCard label="Visits" value="78.4k" percentage={72} color="from-rose-500 to-orange-400" />
            </div>

            <div className="grid grid-cols-10 gap-5">
              <div className="col-span-4 bg-[#1c1642] rounded-3xl p-5 border border-white/5 flex flex-col items-center">
              ...
              <div className="col-span-6 bg-[#1c1642] rounded-3xl p-6 border border-white/5">
  ...
  return (
    <div className="min-h-screen bg-[#120d31] text-white flex p-4 font-sans">
      {/* SIDEBAR */}
      <aside className="w-64 bg-[#1c1642] rounded-3xl p-6 flex flex-col shadow-2xl mr-6">
        <div className="flex items-center gap-2 mb-10 px-2">
          <div className="bg-fuchsia-600 p-1.3 rounded-sm shadow-lg shadow-blue-500/30">
        ...
        <div className="mt-auto bg-linear-to-br from-fuchsia-600 to-purple-700 p-5 rounded-2xl relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col">
        <header className="flex justify-between items-center mb-6 px-2">
          <h2 className="text-1xl font-bold">{activeTab} Overview</h2>
          <div className="flex items-center gap-4 bg-[#1c1642] p-2 rounded-1xl pr-4 border border-white/5">
             <img 
               src="/src/assets/images/hyelzira.jpg"
               alt="User" 
...
const StatCard = ({ label, value, percentage, color }: any) => (
  <div className={`bg-linear-to-br ${color} rounded-2xl p-5 shadow-xl relative overflow-hidden`}>
    <div className="relative z-10 flex flex-col justify-between h-full">
      <p className="text-[10px] uppercase font-bold text-white/70 tracking-widest">{label}</p>
      <div className="mt-4 flex items-end justify-between">
        <h4 className="text-2xl font-black">{value}</h4>
        <div className="text-[10px] bg-white/20 px-2 py-1 rounded-lg backdrop-blur-sm">
          {percentage}%
        </div>
      </div>
    </div>
    <div className="absolute -right-4 -bottom-4 bg-white/10 w-20 h-20 rounded-full blur-2xl"></div>
  </div>
);
